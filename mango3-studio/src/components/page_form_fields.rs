...
pub fn PageFormFields(
    ...
    let value_title = page.as_ref().map(|p| p.title.clone()).unwrap_or_default();
    let value_slug = RwSignal::new(page.as_ref().map(|p| p.slug.clone()).unwrap_or_default());
    let value_content = page.as_ref().map(|p| p.content.clone()).unwrap_or_default();
    let value_cover_image_blob = RwSignal::new(page.as_ref().and_then(|p| p.cover_image_blob.clone()));
    let value_publish = page.map(|p| p.is_published).unwrap_or_default();
