...
use crate::presenters::CursorPagePresenter;
    ...
    Self: Send + Sync,
{
    ...
    fn resource_get(&self) -> Option<CursorPagePresenter<T>>;
    ...
    T: Clone + DeserializeOwned + Send + Serialize + Sync + 'static,
{
    ...
    resource: LocalResource<Result<CursorPagePresenter<T>, ServerFnError>>,
}

impl<T> InfiniteScrollControllerTrait<LocalResource<Result<CursorPagePresenter<T>, ServerFnError>>, T>
    ...
    T: Clone + DeserializeOwned + Send + Serialize + Sync + 'static,
{
    fn new<RF>(resource_fn: RF) -> Arc<Self>
    where
        RF: Fn(RwSignal<Option<Uuid>>) -> LocalResource<Result<CursorPagePresenter<T>, ServerFnError>>,
    {
    ...
    T: Clone + DeserializeOwned + Send + Serialize + Sync + 'static,
{
    ...
    is_loading: RwSignal<bool>,
    pub nodes: RwSignal<Vec<T>>,
    resource: Resource<Result<CursorPagePresenter<T>, ServerFnError>>,
}

impl<T> InfiniteScrollControllerTrait<Resource<Result<CursorPagePresenter<T>, ServerFnError>>, T>
    ...
    T: Clone + DeserializeOwned + Send + Serialize + Sync + 'static,
{
    fn new<RF>(resource_fn: RF) -> Arc<Self>
    where
        RF: Fn(RwSignal<Option<Uuid>>) -> Resource<Result<CursorPagePresenter<T>, ServerFnError>>,
    {
        let after = RwSignal::new(None);
        Arc::new(Self {
            after,
